 ...
 * @short_description: Information about a search source
 * @Title: PhoshSearchSource
 *
 * A #PhoshSearchSource is a ref-counted ( g_rc_box_alloc0() ) #GBoxed type
 * with information about a search source
 */
 ...
 */
PhoshSearchSource *
phosh_search_source_new (const char *id,
                         GAppInfo   *app_info)
{
  PhoshSearchSource *self;

  self = g_rc_box_new0 (PhoshSearchSource);
  self->id = g_strdup (id);
 ...
 */
void
phosh_search_source_set_position (PhoshSearchSource *self,
                                  guint              position)
{
  g_return_if_fail (self != NULL);
...
{
  return g_variant_new ("(ssu)",
                        self->id,
                        g_app_info_get_id (self->app_info),
                        self->position);
}
